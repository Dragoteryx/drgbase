...
function ENT:IsCharging()
  return self:GetNW2Bool("DrGBaseCharging")
end
...
if SERVER then
  ...
  function ENT:Charge(duration, callback)
    ...
    self:SetNW2Bool("DrGBaseCharging", true)
    local now = CurTime()
    self._DrGBaseChargeContact = nil
    while not self:IsDying() and CurTime() < now + duration do
      if callback(CurTime() - now, self._DrGBaseChargeContact) then break end
      self._DrGBaseChargeContact = nil
      self:MoveForward()
      coroutine.yield()
    end
    self:SetNW2Bool("DrGBaseCharging", false)
    self._DrGBaseChargeContact = nil
  end
