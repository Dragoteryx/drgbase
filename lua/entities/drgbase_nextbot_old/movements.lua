...
if SERVER then
  ...
  function ENT:MoveToPos(pos, options, callback)
    ...
    if res == "ok" then self:OnMoveToSuccess(pos, res)
    else self:OnMoveToFailure(pos, res) end
    return res
  end
    ...
    if not navmesh.IsLoaded() then
      ...
      while true do
        ...
        if self:CanMove() and not (options.avoid and self:AvoidObstacles(true)) then
          ...
          if self:GetHullRangeSquaredTo(pos) > options.tolerance^2 then return "ok" end
        end
      ...
      if not IsValid(path) or pos:DistToSqr(path:GetEnd()) > options.tolerance^2 then
        self._DrGBaseComputeReached = path:Compute(self, pos, options.generator)
      end
      ...
      while true do
        ...
        if self:CanMove() then
          ...
          elseif current.type == 5 then
            if not self.ClimbLaddersDown then
              local drop = current.ladder:GetTop().z - current.ladder:GetBottom().z
              if drop > self.loco:GetDeathDropHeight() then return "unreachable"
        ...
        if not IsValid(path) then return "ok"
        elseif self:GetHullRangeSquaredTo(path:GetEnd()) <= options.tolerance^2 then
          self:InvalidatePath()
          return "ok"
        elseif not self._DrGBaseComputeReached and
  ...
  function ENT:FollowEntityMemory(ent, options, callback)
    ...
    if (res == "ok" or res == "unreachable") and self:HasLostEntity(ent) then return "lost"
    else return res end
  end
  ...
  function ENT:FindLedge()
    local tr = self:TraceHull({
      endpos = self:GetPos() + self:GetUp()*999999
    })
    local maxheight = tr.HitPos.z - self:GetPos().z
    if maxheight <= 0 then return end
    local tr2 = {Hit = true}
    local height = 0
    while tr2.Hit and height <= maxheight do
      local start = self:GetPos() + Vector(0, 0, height)
      tr2 = self:TraceHull({
        start = start,
        endpos = start + self:GetForward()*30
      })
      height = height+1
    end
  ...
  function ENT:ClimbLedge(ledge)
    if self:IsClimbing() then return end
    if self:OnStartClimbing(ledge, down) == false then return end
    self:SetNW2Bool("DrGBaseClimbing", true)
    self:SetNW2Bool("DrGBaseClimbingDown", down)
    local offset = self:GetForward()*self.ClimbOffset.x +
    self:GetRight()*self.ClimbOffset.y +
    ...
    local i = 1
    while not self:IsDying() do
      if self:GetPos().z >= ledge.z then break end
      local height = startingHeight + self:GetSpeed()*wait*self:GetScale()*i
      if self:WhileClimbing(ledge, ledge.z - height, false) then break end
      local pos = self:GetPos()
      self:SetPos(Vector(pos.x, pos.y, height) + offset)
      i = i+1
      coroutine.wait(wait)
    end
    self:SetPos(ledge)
    self:OnStopClimbing(ledge, down)
    self:SetNW2Bool("DrGBaseClimbing", false)
    if not self:IsDying() then
      local pos = self:GetPos()
      self:SetPos(navmesh.GetNearestNavArea(pos):GetClosestPointOnArea(pos))
  ...
  function ENT:ClimbWall()
    ...
    if not isvector(ledge) then return end
  ...
  function ENT:UpdateSpeed(run)
    ...
    elseif self:IsCharging() then return self.ChargeSpeed
    elseif run then return self.RunSpeed
    else return self.WalkSpeed end
  end

  function ENT:OnMoveToSuccess() end
  function ENT:OnMoveToFailure() end
